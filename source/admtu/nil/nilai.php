...
$ke = "$filenya?tapelkd=$tapelkd&smtkd=$smtkd&kelkd=$kelkd&".
			"progkd=$progkd&rukd=$rukd&skkd=$skkd&mapelkd=$mapelkd&page=$page";
...
if ($_POST['btnSMP'])
	...
	if ($tgk != 0)
		...
		{
		//insert
		mysql_query("INSERT INTO siswa_rangking(kd, kd_tapel, kd_program, kd_kelas, kd_ruang, ".
						"kd_siswa_kelas, kd_smt, total_kognitif, rata_kognitif, ".
						"total_psikomotorik, rata_psikomotorik, total) VALUES ".
						"('$x', '$tapelkd', '$progkd', '$kelkd', '$rukd', ".
	...
	//re-direct
	$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&progkd=$progkd&rukd=$rukd&smtkd=$smtkd&skkd=$skkd&mapelkd=$mapelkd";
	xloc($ke);
	exit();
	}
...
if ($_POST['btnSMP2'])
	...
	if (empty($ekstra))
		...
		$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&progkd=$progkd&rukd=$rukd&smtkd=$smtkd&skkd=$skkd";
		pekem($pesan,$ke);
		exit();
		}
	else
...
do
	...
	echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&rukd='.$rukd.'&smtkd='.$smtkd.'&skkd='.$std_skkd.'">'.$std_no.'. '.$std_nis.'. '.$std_nama.'</option>';
	}
...
if (empty($tapelkd))
	...
	{
	echo '[<a href="nilai_pdf.php?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&rukd='.$rukd.'&smtkd='.$smtkd.'&skkd='.$skkd.'" target="_blank" title="Lihat Raport..."><img src="'.$sumber.'/img/pdf.gif" width="16" height="16" border="0"></a>]
	<hr>
	<p>
	<big>
	...
	if ($stdx_mulo == "true")
		{
		echo '<option value="'.$stdx_kd.'">Muatan Lokal --> '.$stdx_pel.'</option>';
		}
	else
		{
	...
	do
		...
		if ($std_mulo == "true")
			{
			echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&rukd='.$rukd.'&smtkd='.$smtkd.'&skkd='.$skkd.'&mapelkd='.$std_kd.'">Muatan Lokal --> '.$std_pel.'</option>';
			}
		else
			{
			echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&rukd='.$rukd.'&smtkd='.$smtkd.'&skkd='.$skkd.'&mapelkd='.$std_kd.'">'.$std_pel.'</option>';
			}
		}
	while ($rowstd = mysql_fetch_assoc($qstd));
